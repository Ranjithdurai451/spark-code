...
export default function SidePanel({ isVisible, showPanel }: SidePanelProps) {
  ...
  // Chat hooks with unified error handling
  const {
    messages,
    append,
    ...
    onError: async (error) => {
      ...
      let errorSuggestion = "Please try again";
      ...
      try {
        ...
        if (errorContent.success === false && errorContent.error) {
          ...
          if (
            ...
            clearApiKeys();
            errorSuggestion = "Please set up your API keys in settings";
          }
        ...
        errorMessage = error.message || "Failed to analyze code";
      }
  ...
  const {
    ...
    onError: (error) => {
      ...
      try {
        ...
        } else if (errorContent.error || errorContent.message) {
          // Fallback for old format during transition
          errorMessage =
            errorContent.message || errorContent.error || errorMessage;
          if (errorContent.error === "NO_API_KEY") {
            clearApiKeys();
            errorSuggestion = "Please set up your API keys in settings";
          }
  ...
  const isAnalyzing =
    analysisLoading ||
    ["loading", "streaming", "submitted", "in_progress"].includes(
      status as any,
    );
  ...
  async function handleAnalyze() {
    ...
    if (!code || code.trim().length < 10) {
      const validationError = {
        ...
        suggestion:
          "Please provide at least 10 characters of code with some logic",
        category: "validation",
      };
  ...
  const actions = [
    {
      ...
      description:
        "AI-powered code analysis with quality insights and optimization suggestions",
      icon: Brain,
      accentIcon: Sparkles,
    ...
    },
    {
      ...
      description:
        "Generate comprehensive test suites with edge cases and validation",
      icon: TestTube2,
      accentIcon: Target,
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {action.hasContent &&
                                  ...
                                  !action.hasError && (
                                    <Badge
                                      variant="secondary"
                                      className="text-xs py-0 h-5 bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400"
                                    >
