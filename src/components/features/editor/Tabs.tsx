...
import { Plus, X, Edit2, Code, MoreHorizontal, MoreVertical } from "lucide-react";
      ...
      <div className="relative border-b border-border bg-muted/50">
        <div className="flex overflow-x-auto scrollbar-thin scrollbar-thumb-muted/60 scrollbar-track-transparent">
          {tabs.map((tab) => (
            ...
            >
              <span className="overflow-hidden text-sm text-ellipsis whitespace-nowrap flex-1 mr-1">
                {tab.name}
              </span>
              <span className="text-xs opacity-60 capitalize px-1 py-0.5 bg-muted/60 rounded text-[10px] font-medium">
                {tab.language}
              </span>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    className="ml-1.5 p-1 rounded hover:bg-accent/60 opacity-0 group-hover:opacity-100 transition-all duration-200 focus-visible:opacity-100 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1"
                    onClick={(e) => {
                      e.stopPropagation();
                    }}
                  >
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: Language) => {
                  setNewLang(value);
                  if (error) setError(null); // Clear error on language change
                }}
              >
                <SelectTrigger>
                  <SelectValue />
