...
export async function POST(req: NextRequest) {
  try {
    ...
    if (repoResponse.ok) {
      ...
      if (!repositoryData.permissions?.push && !repositoryData.permissions?.admin) {
        return NextResponse.json({ 
      ...
      return NextResponse.json({ 
        error: "Unable to access repository. Check permissions and repository name." 
      }, { status: 403 });
    }
    ...
    if (!repoExists) {
      const createRepoResponse = await fetch("https://api.github.com/user/repos", {
        ...
        body: JSON.stringify({
          name: repo,
          description: "DSA solutions and code snippets created with SparkCode",
          auto_init: true,
          private: false
        })
      });
      ...
      if (!createRepoResponse.ok) {
        ...
        console.error("Repository creation failed:", createRepoResponse.status, createError);
    ...
    if (!fileOperationResponse.ok) {
      ...
      console.error("File operation failed:", fileOperationResponse.status, operationError);
      ...
      try {
        ...
        return NextResponse.json({ 
          error: "Failed to save file to GitHub. Please check your permissions." 
        }, { status: 500 });
      }
    }
